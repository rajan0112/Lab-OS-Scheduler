...
class PREPRIOScheduler : public BasicScheduler {
public:
	PREPRIOScheduler(int maxPriority) {
...
//		expired = new std::array<std::queue<Process*>, 3>;
	...
	PREPRIOScheduler(const PREPRIOScheduler&) = delete; 

	void add_process(Process *p);
	int get_scheduler_algo () { return 5; } // FCFS=0, LCFS=1, SRTF=2, RR=3, PRIO=4, PREPRIO=5
